...
if len(sys.argv) != 5:
    logger.error("Usage: oltp_to_clickhouse.py <target_table> <ch_jdbc_url> <user> <password>")
    sys.exit(1)
...
spark = SparkSession.builder \
    .appName(f"Kafka2ClickHouse_{target_table}") \
...
def write_to_clickhouse(df: DataFrame, epoch_id: int):
    try:
        ...
        df.write \
            ...
            .option("dbtable", f"default.{target_table}") \
            .option("user", ch_user) \
            .option("password", ch_password) \
            .option("driver", "com.clickhouse.jdbc.ClickHouseDriver") \
            .option("customSchema", "trade_id String, price Double, quantity Double, trade_time Timestamp, is_buyer_maker Boolean, loaded_at Timestamp") \
...
logger.info(f"🎯 Spark Streaming started! Topic: {KAFKA_TOPIC} → ClickHouse: default.{target_table}")
